...
static const uint8_t KeyCodes[KEY_CODE_MAX+1] = {
	...
	0x00, 0x00, 0xf1, 0xf0, 0x54, 0x61, 0x60, 0x5e, 0x5d, 0x5a, 0x5b, 0x62, 0x85, 0x63, 0, 0,
...
{
	if (kc <= KEY_CODE_MAX) {
		LED_Set_Key_RGB(kc, 0x1800, 0x1800, 0x1800);
		kc = KeyCodes[kc];
		if (!kc)
			;
		else if (kc < 0xe0) {
...
void TIM_EncoderCallback(uint8_t value)
{
	...
	LED_Set_Key_RGB(KEY_CODE_QBUT, r<<5, g<<5, b<<5);
}
...
void USB_HIDOutReportCallback(const uint8_t *report)
{
	...
	for (i=0; i<4; i++) {
		if (mask & 1)
			LED_Set_LED_RGB(LED_id[i], 0x1800, 0x1800, 0x1800);
		else
